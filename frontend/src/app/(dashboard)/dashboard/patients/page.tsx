  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-slate-100">Pacientes</h1>
        <Button onClick={() => { resetForm(); setShowModal(true); }}>Nuevo Paciente</Button>
      </div>

      <div className="mt-6 flex gap-3">
        <Input placeholder="Buscar por nombre o microchip..." value={search} onChange={(e) => setSearch(e.target.value)} className="max-w-md" />
        <select
          value={speciesFilter}
        ...
        <table className="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
          <thead className="bg-gray-50 dark:bg-slate-700/50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium uppercase text-gray-500 dark:text-slate-400">Especie / Raza</th>
              <th className="px-6 py-3 text-left text-xs font-medium uppercase text-gray-500 dark:text-slate-400">Nacimiento</th>
              <th className="px-6 py-3 text-left text-xs font-medium uppercase text-gray-500 dark:text-slate-400">Peso</th>
          ...
          <tbody className="divide-y divide-gray-200 dark:divide-slate-700">
            {loading ? (
              ...
              <tr><td colSpan={6} className="px-6 py-4 text-center text-gray-500 dark:text-slate-400">No hay pacientes</td></tr>
            ) : (
              patients.map((p) => (
                <tr key={p.id} className={`hover:bg-gray-50 dark:hover:bg-slate-700/30 ${!p.is_active ? "opacity-50" : ""}`}>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <span className="text-xl">{speciesIcon(p.species)}</span>
                      <Link href={`/dashboard/patients/${p.id}`} className="text-sm font-medium text-gray-900 dark:text-slate-100 hover:text-primary-600 dark:hover:text-primary-400">
                        {p.name}
                      </Link>
                    </div>
                  </td>
                  <td className="px-6 py-4 text-sm text-gray-500 dark:text-slate-400">{p.species}{p.breed ? ` — ${p.breed}` : ""}</td>
                  <td className="px-6 py-4 text-sm text-gray-500 dark:text-slate-400">{p.birth_date || "-"}</td>
                  <td className="px-6 py-4 text-sm text-gray-500 dark:text-slate-400">{p.weight ? `${p.weight} kg` : "-"}</td>
                  <td className="px-6 py-4">

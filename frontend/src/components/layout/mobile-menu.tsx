...
function NavItem({ item, onClose }: { item: { label: string; href: string; icon: string }; onClose: () => void }) {
  ...
  return (
    <>
      ...
      <button
        ...
        className="md:hidden flex flex-col gap-1.5 p-2 hover:bg-gray-100 dark:hover:bg-slate-700/50 rounded-lg transition-colors"
        aria-label="Toggle menu"
      >
        <span className="w-6 h-0.5 bg-gray-900 dark:bg-slate-100 transition-all" />
        <span className="w-6 h-0.5 bg-gray-900 dark:bg-slate-100 transition-all" />
        <span className="w-6 h-0.5 bg-gray-900 dark:bg-slate-100 transition-all" />
      </button>

      {/* Mobile menu sheet */}
      <Sheet open={open} onClose={handleClose} side="left">
        <div className="flex h-screen flex-col bg-white dark:bg-slate-800">
          {/* Logo */}
          ...
          <nav className="flex-1 overflow-y-auto p-4 space-y-4">
            <ul className="space-y-0.5">
              {visibleMenuItems.map((item) => (
                <NavItem key={item.href} item={item} onClose={handleClose} />
              ))}
            </ul>

            {isSuperAdmin() && (
              <div>
                <p className="px-4 pt-2 pb-1 text-xs font-semibold uppercase tracking-wider text-gray-400 dark:text-slate-500">
                  SuperAdmin
                </p>
                <ul className="space-y-0.5">
          ...
          <div className="border-t border-gray-200 p-4 dark:border-slate-700">
            <div className="mb-3 rounded-lg bg-gray-50 px-4 py-3 dark:bg-slate-700/50">
            ...
            <button
              ...
              className="w-full rounded-lg px-4 py-2 text-left text-sm font-medium text-red-600 hover:bg-red-50 transition-colors dark:text-red-400 dark:hover:bg-red-900/20"
            >
              Cerrar sesión
            </button>
          </div>
